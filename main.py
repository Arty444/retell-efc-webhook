"""
AI Voice Agent Webhook Server
Extended to write call data to Supabase alongside existing Zapier flow.

Deploy: Push to github.com/Arty444/retell-efc-webhook (main branch)
...
# CORS — allow the React dashboard to call API endpoints
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "http://localhost:5173",
...
if SUPABASE_URL and SUPABASE_SERVICE_KEY:
    ...
    logger.warning("Supabase credentials not set — call data will NOT be saved to database")
...
async def forward_to_zapier(payload: dict) -> None:
    if not ZAPIER_WEBHOOK_URL:
        logger.warning("ZAPIER_WEBHOOK_URL not set, skipping Zapier forward")
        return
    try:
        async with httpx.AsyncClient() as client:
            resp = await client.post(ZAPIER_WEBHOOK_URL, json=payload, timeout=10)
            logger.info(f"Zapier response: {resp.status_code}")
    except Exception as e:
        logger.error(f"Zapier forward failed: {e}")


def write_to_supabase(data: dict, client_id: str) -> None:
    if not supabase:
    ...
    try:
        call_result = supabase.table("calls").insert(call_record).execute()
        inserted_call_id = call_result.data[0]["id"] if call_result.data else None
        logger.info(f"Call written to Supabase: {inserted_call_id}")
        ...
        if is_lead and inserted_call_id:
            ...
            logger.info(f"Lead created for call {inserted_call_id}")
...
async def retell_webhook(request: Request):
    ...
    logger.info(f"Processing call from {data['from_number']} — agent {data['agent_id']}")
    ...
    if client_id:
        ...
        logger.warning(f"No client_id mapping for agent {data['agent_id']} — skipping Supabase write")
...
async def crm_webhook(request: Request):
    ...
    body = await request.json()
    logger.info(f"CRM webhook received: {body.get('event_type', 'unknown')}")
    ...
    try:
        supabase.table("crm_events").insert({
            "client_id": client_id,
        ...
        logger.info(f"CRM event stored: {event_type} for {contact_name}")

        if event_type in ("member_joined", "converted", "payment_received") and contact_phone:
            result = supabase.table("leads") \
                .update({"status": "Converted"}) \
                .eq("client_id", client_id) \
                .eq("caller_phone", contact_phone) \
                .eq("status", "Booked") \
                .execute()

            if result.data:
                logger.info(f"Lead auto-converted for {contact_phone}")
...
After pasting that, also open **`requirements.txt`** and replace it with:
```
fastapi
uvicorn[standard]
httpx
